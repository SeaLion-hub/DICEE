...
| **일정 저장**         | [데이터베이스 명세서 v7.0](decisions/database-spec.md)의 notice_schedules·active_notice_schedules_mv에 따라 구현. 공지-일정 1:N 정규화(Option A).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| **DB 스키마·SSOT**    | DDL·ORM·마이그레이션·인프라는 [데이터베이스 명세서 v7.0](decisions/database-spec.md)를 단일 진실 공급원(SSOT)으로 따른다. 타협 없이 반영.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
각 단계 진입 전 또는 해당 단계 설계 시 결정해야 할 사항. 상세 비교·결정 근거는 [데이터베이스 명세서](decisions/database-spec.md) 등 참고.
...
**5단계 계약 (매칭·페이지네이션·달력·툼스톤)**: [decisions/user-notice-matching-and-api-contracts.md](decisions/user-notice-matching-and-api-contracts.md) (**APPROVED** 2026-03-27). **5단계 코어 API는 코드 기준 반영 완료**(WORK_LOG 2026-03-29). FTS·GIN 키워드 검색만 후속 PR로 남음.
...
| **일정 스키마·5단계 성능**       | Notice가 dates(JSONB)만 있으면 날짜 범위 쿼리 복잡·달력 API 병목.                                                     | [데이터베이스 명세서](decisions/database-spec.md) 참고. notice_schedules 정규화·Materialized View 적용.                                                     |
...
| 5단계 | 완료(코어)   | 코어(매칭·달력·ICS·일정 동기화)는 코드 기준 반영 완료. **PostgreSQL FTS·GIN 키워드 검색**만 후속 PR(아래 구현 현황) |
| 6단계 | 3주       | —                 |
...
**할 일 요약**: 로컬·Railway DATABASE_URL. PostgreSQL + psycopg. SQLAlchemy 2.0으로 Notice, College, User 정의. Raw vs 정제: 같은 테이블(Notice)에 단계별 컬럼. 일정 저장은 [데이터베이스 명세서](decisions/database-spec.md)의 notice_schedules·MV에 따라 구현. content_hash, is_manual_edited 미리 설계. **Auth**: 구글 로그인(OAuth) 먼저, JWT 발급. 구글 ID 토큰 서명 검증(verify_signature=False 금지). User: provider, provider_user_id, 프로필 필드. OAuth 핸드쉐이크: 응답 body JSON으로 Access/Refresh JWT, 프론트는 Authorization 헤더. 프론트에서 구글 OAuth 후 Authorization Code를 백엔드 `/v1/auth/google`로 전달 → 백엔드가 구글 검증 후 자체 JWT 발급. Alembic 마이그레이션 세팅.
...
**진행 현황 (2026-03-19)**:
- [x] Stage 1/Stage 2 프롬프트를 Deep Research 최종안 방향으로 개편 (`app/services/ai/extractor.py`)
- [x] Stage 2에서 `preselected_main_categories`를 불변 상수로 재사용하도록 지시 강화
...
- [x] `GET`/`PATCH /v1/users/me`, `GET /v1/meta/department-options`·`grade-options`, 맞춤 피드 `GET /v1/notices/matched` — 계약: [user-notice-matching-and-api-contracts.md](decisions/user-notice-matching-and-api-contracts.md) (`app/api/v1/users.py`, `meta.py`, `notices_matched.py`, `app/data/department_catalog.json`)
- [x] `GET /v1/calendar/events`, `GET /v1/calendar/feed.ics`, `POST`/`DELETE /v1/users/me/calendar/events/...`, AI 후 `notice_schedules` 동기화 (`app/api/v1/calendar.py`, `app/services/tasks.py` 등). Google Calendar OAuth·웹훅은 6단계 이후
