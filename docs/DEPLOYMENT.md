...
- **구현**: FastAPI에 **POST /internal/trigger-crawl** 엔드포인트 추가. 요청 시 **보안 키**(헤더 예: `X-Admin-Key` 또는 쿼리) 검증. 검증 통과 시 Celery 크롤 태스크 enqueue. Cron이 1시간마다(또는 정한 주기로) 위 URL을 호출.
