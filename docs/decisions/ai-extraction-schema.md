...
| 3 | 공지 카테고리 플래그 `NoticeCategory` | **반영** | DB notices 테이블에 `category` VARCHAR(64) 존재. 6단계 탭/필터 및 "모집 vs 안내" 구분에 필수. |
...
- **NoticeCategory**  
  - `RECRUITMENT` (모집/지원: 장학금, 인턴, 동아리 등)  
  - `INFO` (안내: 휴강, 식단, 서버 점검 등)  
  - `EVENT` (행사: OT, 설명회 등)  
  - `OTHER`
- **ScheduleKind**  
  - 기존 유지 (APPLICATION_DEADLINE, INTERVIEW, RESULT, EVENT, OTHER)
- **TargetGrade** (학년)  
  - `Literal["1","2","3","4","5","6","all"]` 또는 소규모 Enum.  
  - "3+"는 `"3"` + 별도 필드(예: `grade_min_only`) 또는 매칭 시 "3 이상" 해석으로 처리.
- **TargetDepartment**  
...
- **date_raw: str | None**  
  - AI가 원문 그대로 옮기는 필드 ("11월 중순", "추후 공지" 등).  
  - DB `notice_schedules.schedule_text_fallback`와 1:1 매핑.
- **model_validator**  
...
- **notice_category: NoticeCategory**  
  - 필수 또는 기본값 `OTHER`. 6단계 탭/필터 및 매칭 전 필터링에 사용.
- **schedules**  
...
- **target_grades**: `list[TargetGrade]` 제한.
- **target_departments**: 1안 Enum, 2안 `list[str]` + 프롬프트 정규화 후 추후 Enum 전환.
...
- **DB 매핑**:  
  - `notice_schedules.schedule_text_fallback` ← `ScheduleItem.date_raw`  
  - `notices.category` ← `NoticeAIExtraction.notice_category.value`
